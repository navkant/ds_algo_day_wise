...
def solve(A: List[int]):
        ...
        for i in range(len(A)):
            no_of_lesser_elements = i

            if i == n-1:
                no_of_greater_elements = 0
            else:
                no_of_greater_elements = n - 1 - i

            contri_as_min = A[i] * (2 ** (no_of_greater_elements))
            contri_as_max = A[i] * (2 ** (no_of_lesser_elements))
            total_contribution += (contri_as_max-contri_as_min)
        print(total_contribution)
        return total_contribution % 1000000007
